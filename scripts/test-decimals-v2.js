...
async function main() {
    console.log("Testing Token Sale Decimal Handling (v2)\n");

    // Test cases with 6 decimal precision
    const testCases = [
    ...
    for (const test of testCases) {
        ...
        const usdtAmountBN = ethers.utils.parseUnits(test.usdt, 6);
        
        // Calculate tokens: (USDT amount * 10^18) / token price (20000)
        const tokenAmountBN = usdtAmountBN.mul(ethers.constants.WeiPerEther).div(20000);
        
        console.log(`\nInput USDT: ${test.usdt}`);
        console.log(`USDT Amount (6 decimals): ${usdtAmountBN.toString()}`);
